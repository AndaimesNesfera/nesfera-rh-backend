...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    if (!adminOk) {
      const { data: usuarios } = await supabase.from('usuarios').select('data');
      if (usuarios && Array.isArray(usuarios)) {
        const match = usuarios.find(row => {
          const u = row.data;
          return u && u.ativo !== false && u.username === username && u.hash === hash;
        });
        if (match) { adminOk = true; loginNome = match.data.nome || match.data.username; }
      }
    }
    if (!adminOk) return res.status(401).json({ error: 'Usuário ou senha incorretos' });
    const token = jwt.sign({ username, nome: loginNome }, process.env.JWT_SECRET, { expiresIn: '8h' });
...
app.get('/api/:store', authMiddleware, async (req, res) => {
  ...
  if (!VALID_STORES.includes(store)) return res.status(400).json({ error: 'Store inválido' });
  try {
    const { data, error } = await supabase.from(store).select('id, data');
    if (error) throw error;
    res.json((data || []).map(row => ({ id: row.id, ...row.data })));
...
app.get('/api/:store/:id', authMiddleware, async (req, res) => {
  ...
  if (!VALID_STORES.includes(store)) return res.status(400).json({ error: 'Store inválido' });
  try {
    const { data, error } = await supabase.from(store).select('id, data').eq('id', id).single();
    if (error) { if (error.code === 'PGRST116') return res.status(404).json({ error: 'Não encontrado' }); throw error; }
...
app.post('/api/:store', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const { data, error } = await supabase.from(store).insert({ data: dataFields }).select('id, data').single();
    if (error) throw error;
...
app.put('/api/:store/upsert', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const { data, error } = await supabase.from(store).upsert({ id, data: dataFields }, { onConflict: 'id' }).select('id, data').single();
    if (error) throw error;
    res.json({ id: data.id, ...data.data });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
  ...
  try {
    ...
    if (Array.isArray(attachments)) {
      for (const att of attachments) {
        ...
        mailAttachments.push({ filename: att.filename || 'documento.pdf', content: Buffer.from(base64Data, 'base64'), contentType: att.mimeType || 'application/octet-stream' });
      }
    }
    ...
    await transporter.sendMail({
      ...
      subject: subject || 'Documentos Nesfera — ' + (toName || to),
      text: message,
      html: message ? '<div style="font-family:Arial,sans-serif">' + message.replace(/\n/g, '<br>') + '</div>' : undefined,
...
app.post('/api/send-whatsapp', authMiddleware, async (req, res) => {
  ...
  try {
    const results = [];
    if (message) {
      const textResp = await fetch(EVOLUTION_URL + '/message/sendText/' + EVOLUTION_INST, {
        method: 'POST', headers,
        body: JSON.stringify({ number: phoneWithCountry, text: message })
      });
      const textData = await textResp.json();
      if (!textResp.ok) throw new Error('Falha WhatsApp texto: ' + JSON.stringify(textData));
    ...
    if (Array.isArray(attachments) && attachments.length > 0) {
      for (const att of attachments) {
        ...
        const mediaResp = await fetch(EVOLUTION_URL + '/message/sendMedia/' + EVOLUTION_INST, {
          method: 'POST', headers,
          body: JSON.stringify({ number: phoneWithCountry, mediatype: getMediaType(mimeType), mimetype: mimeType, caption: att.filename || '', media: base64Data, fileName: att.filename || 'documento' })
        });
        const mediaData = await mediaResp.json();
        if (!mediaResp.ok) { results.push({ type: 'media', filename: att.filename, status: 'error', error: JSON.stringify(mediaData) }); }
        else { results.push({ type: 'media', filename: att.filename, status: 'sent' }); }
      }
    }
...
app.get('/api/whatsapp/status', authMiddleware, async (req, res) => {
  ...
  const EVOLUTION_INST = process.env.EVOLUTION_INSTANCE;
  const EVOLUTION_KEY  = process.env.EVOLUTION_API_KEY;
  if (!EVOLUTION_URL || !EVOLUTION_INST || !EVOLUTION_KEY) return res.json({ connected: false, reason: 'Não configurado' });
  try {
    const resp = await fetch(EVOLUTION_URL + '/instance/connectionState/' + EVOLUTION_INST, { headers: { 'apikey': EVOLUTION_KEY } });
...
app.get('/api/whatsapp/qrcode', authMiddleware, async (req, res) => {
  ...
  const EVOLUTION_KEY  = process.env.EVOLUTION_API_KEY;
  try {
    const resp = await fetch(EVOLUTION_URL + '/instance/connect/' + EVOLUTION_INST, { headers: { 'apikey': EVOLUTION_KEY } });
...
async function initDatabase() {
  try {
    ...
    if (!data) {
      ...
      console.log('Config padrao criada (admin/Ne1505@15)');
    } else {
      console.log('Config ja existe no banco');
    }
  } catch (e) {
...
app.listen(PORT, async () => {
  console.log('Nesfera RH Backend rodando na porta ' + PORT);
